...
export function ToolsTabs({ categories }: ToolsTabsProps) {
  return (
    <Tabs defaultValue="all" className="w-full">
      ...
      {/* All tools — shows every category sequentially */}
      <TabsContent value="all" className="mt-0">
        {categories.map((cat) => (
          <div key={cat.id} className="max-w-container mx-auto px-6 py-10 border-b border-border-default last:border-b-0" id={`all-${cat.id}`}>
            <h2 className="text-[1.5rem] font-bold text-brand-ink">{cat.label}</h2>
            <p className="text-[1rem] text-text-secondary leading-[1.75] max-w-2xl mt-2 mb-6">
              {cat.description}
            </p>
